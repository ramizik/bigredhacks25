...
def generate_kid_friendly_image(story_text: str, scene_context: str = "", age_group: str = "5-8") -> Dict:
    ...
    try:
        ...
        enhanced_prompt = f"""
        Children's book illustration based on: {story_text}
        ...
        Safety: High safety filter, appropriate for children ages {age_group}
        """
        
        print(f"🎨 Enhanced prompt created for Imagen 3.0")
        
        # Generate image using Imagen 3.0 - following dd project pattern
        response = client.models.generate_images(
            model="imagen-3.0-generate-002",
            prompt=enhanced_prompt,
            config=types.GenerateImagesConfig(
                number_of_images=1,
                aspect_ratio="16:9",  # Good for story illustrations
                safety_filter_level="block_low_and_above",  # High safety for kids
                person_generation="allow_adult"  # Allow characters but keep safe
            )
        )
        
        # Debug the response structure
        print(f"🔍 Imagen API Response Debug:")
        print(f"   Response type: {type(response)}")
        print(f"   Has generated_images attr: {hasattr(response, 'generated_images')}")
        if hasattr(response, 'generated_images'):
            print(f"   Generated images count: {len(response.generated_images) if response.generated_images else 0}")
            print(f"   Generated images value: {response.generated_images}")
        ...
        if response.generated_images:
            ...
            return {
                ...
                "debug_info": {
                    ...
                    "has_generated_images": hasattr(response, 'generated_images')
                }
            }
